...
import <%= model_name.classify %> from '../../resource_models/<%= model_name.underscore %>'
...
export const all = ({ dispatch, commit }, options) => {
  ...
  return new Promise((resolve, reject) => {
    <%= model_name.classify %>.all(options)
      .then(response => {
...
export const destroy = ({ dispatch, commit }, model) => {
  ...
  return new Promise((resolve, reject) => {
    model.destroy()
      .then(response => {
        commit(types.DELETE_<%= model_name.underscore.upcase %>_SUCCESS, model.id)
...
export const receiveResourcesFromRelationships = ({ commit }, response) => {
  return new Promise((resolve, reject) => {
    commit(types.GET_RELATED_<%= model_name.underscore.pluralize.upcase %>_SUCCESS, response)
